...
void Preprocess::pandar_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    for (int i = 0; i < plsize; i++)
    {
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        // offset time relative to first point, converted to ms (curvature field stores time offset)
        ...
        if (dist < blind * blind || std::isnan(added_pt.x) || std::isnan(added_pt.y) || std::isnan(added_pt.z))
            continue;
